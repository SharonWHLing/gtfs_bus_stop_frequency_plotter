...
for (r in gtfs_route_id_only$route_id){
  ...
  r_table_AM <- r_table %>% filter(departure_time < ("10:00:00"))
  # r_table_PM <- r_table %>% filter(departure_time >= ("15:00:00") & departure_time < ("20:00:00")) 
  # r_table_Other <- r_table %>% filter(departure_time >= ("10:00:00") & departure_time < ("15:00:00") | departure_time >= ("20:00:00"))
  
  # create table of unique stop IDs for a particular route only
  # r_table_AM_stop_id_only <- r_table_AM[,c("stop_id")] %>% unique() #use for alternative V1 
